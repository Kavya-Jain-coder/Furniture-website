import React from "react";

const Banner = ({ data }) => {
  return (
    <div className="min-h-[550px] flex justify-center items-center py-12 ">
      <div className="mx-auto px-[1rem] sm:px-[10.2rem]">
        <div
          className={`grid grid-cols-1 md:grid-cols-3 gap-6 items-center text-gray-900 rounded-3xl ${data.bgGradient}`}
        >
          <div className="p-6 sm:p-8">
            <p data-aos="slide-right" className="text-sm">
              {data.discount}
            </p>
            <h1
              data-aos="zoom-out"
              className="uppercase text-4xl lg:text-7xl font-bold"
            >
              {data.title}
            </h1>
            <p data-aos="fade-up" className="text-sm">
              {data.date}
            </p>
          </div>

          <div data-aos="zoom-in" className="h-full flex items-center">
            <img
              src={data.image}
              alt="chair-image"
              className="scale-135 w-[250px] md:w-[340px] mx-auto drop-shadow-2xl object-cover -rotate-y-180 "
            />
          </div>

          <div className="flex flex-col justify-center gap-4 p-6 sm:p-8 object-cover">
            <p data-aos="zoom-out" className="font-bold text-xl">
              {data.title2}
            </p>
            <p data-aos="fade-up" className="text-3xl sm:text-4xl font-bold">
              {data.title3}
            </p>
            <p data-aos="fade-up" className="text-sm tracking-wide leading-5">
              {data.title4}
            </p>
            
          </div>
        </div>
      </div>
    </div>
  );
};

export default Banner;
